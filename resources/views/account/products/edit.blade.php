        ...
        <form action="{{ route('account.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <section class="rounded-[2rem] border border-white/10 bg-slate-950/40 p-6 lg:p-8">
                ...
                <div class="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
                    ...
                    <div class="space-y-5">
                        <div><label class="mb-2 block text-sm font-medium text-slate-200">{{ $isEnglish ? 'Trading Type' : 'Jenis Trading' }}</label><select name="trade_kind" class="imtgt-input" required><option value="goods" @selected(old('trade_kind', 'goods') === 'goods')>{{ $isEnglish ? 'Imported Goods' : 'Import Barang' }}</option><option value="services" @selected(old('trade_kind') === 'services')>{{ $isEnglish ? 'Imported Services' : 'Import Jasa' }}</option></select></div>

                        <div class="grid gap-5 sm:grid-cols-2"><div><label class="mb-2 block text-sm font-medium text-slate-200">{{ $isEnglish ? 'Origin Country' : 'Negara Asal' }}</label><input name="origin_country" value="{{ old('origin_country') }}" class="imtgt-input">@error('origin_country')<p class="mt-2 text-sm text-rose-300">{{ $message }}</p>@enderror</div><div><label class="mb-2 block text-sm font-medium text-slate-200">{{ $isEnglish ? 'Min Order Qty' : 'Min Order Qty' }}</label><input name="min_order_qty" value="{{ old('min_order_qty') }}" class="imtgt-input">@error('min_order_qty')<p class="mt-2 text-sm text-rose-300">{{ $message }}</p>@enderror</div></div>
